...
If we publish a $0.50/cold-call cost and the API runs at $0.80 average, our gross margin on Scale's max-included usage shrinks to 20%. We monitor cold-call cost weekly via internal telemetry. If the average is structurally higher than expected for a sustained period, we'll either lower the included-cold number or raise the base price. We'd telegraph any pricing change with 30 days notice on existing contracts.
