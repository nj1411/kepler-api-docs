...
The 202 typically arrives in under 1 second — it's just the time to start the scoring pipeline and write the job row.
...
stateDiagram-v2
  ...
  pending --> complete: pipeline SUCCEEDED
  pending --> failed: pipeline FAILED / TIMED_OUT / ABORTED
  complete --> [*]: 30-day TTL
  failed --> [*]: 30-day TTL
```
...
Before 2026-05-12, `POST /v1/score` blocked synchronously for up to 60 seconds on cold paths. This was incompatible with the API gateway's request timeout — cold requests would intermittently return 503 while the scoring engine continued running server-side. Auto-queue resolves this. The `wait=false` parameter is still accepted for backwards compatibility but is now a no-op — cold is always async, sync `wait=true` requests get the same 202 response shape.
