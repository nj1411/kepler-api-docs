...
`POST /v1/score?wait=false` is available on **Growth** tier and above. Free / Starter callers receive `400 { error: "async_requires_growth" }` if they request it. See [Async scoring](/endpoints/async) for the full polling loop.
